...
### Rationale (Updated: Native ARM64 Runners)
1. **Native execution**: GitHub now provides `ubuntu-24.04-arm` runners (free for public repos)
2. **10-50x speedup**: Native builds vs. QEMU emulation (~2min vs ~20min)
3. **Podman-static**: v5.6.2 provides heredoc support (buildah >= 1.35.0) without system package upgrade
4. **3-stage pipeline**: `prepare` → `build` (matrix) → `manifest` ensures proper dependency ordering

### Alternatives Considered
| Alternative | Why Chosen / Rejected |
|-------------|------------------|
| QEMU emulation with single job | ❌ Rejected: 10-50x slower than native ARM64 runners |
| Docker buildx with buildkit | ❌ Rejected: Incompatible with podman requirement (FR-004) |
| Native runners (amd64 + arm64 runners) | ✅ **CHOSEN**: Free for public repos, massive performance improvement |
...
2. **arm64 build performance**: Native ARM64 runners vs. QEMU emulation
   - **Status**: ✅ Resolved - Using native `ubuntu-24.04-arm` runners (free for public repos)
   - **Result**: 10-50x performance improvement (~2min vs ~20min builds)
...
- Native runners: Matrix strategy with `ubuntu-24.04` (amd64) and `ubuntu-24.04-arm` (arm64)
- Podman-static: v5.6.2 for heredoc support (buildah >= 1.35.0)
- **Adoption**: 3-stage pipeline with parallel native builds, then multi-arch manifest creation
