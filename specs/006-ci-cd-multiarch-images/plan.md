...
- redhat-actions/buildah-build@v2 (rootless container builds)
- redhat-actions/push-to-registry@v2 (registry publishing)
- docker/setup-qemu-action@v3 (multi-arch emulation)

**Storage**: GitHub Actions cache for build layers
**Testing**: Manual validation via workflow dispatch, automated integration tests via push/tag triggers
**Target Platform**: GitHub Actions runners (Linux x86_64), output images for linux/amd64 and linux/arm64
**Project Type**: Infrastructure/CI-CD (workflow configuration, no application code)
**Performance Goals**:
- Clean builds complete within 15 minutes
- Cached builds complete in ≤7.5 minutes (50% reduction target)
- Registry push operations complete within 3 minutes per registry
...
3. **Challenge**: QEMU emulation for arm64 on amd64 runners adds significant build time
   **Mitigation**: Parallel architecture builds where possible; accept longer initial build time (~15min) as cached builds hit 50% target
...
1. **GitHub Secrets**:
   ...
   - `DOCKERHUB_TOKEN`: Docker Hub Personal Access Token with `repo:delete` scope
...
```bash
# Local build (requires podman with qemu-user-static)
