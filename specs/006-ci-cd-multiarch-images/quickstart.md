...
| `DOCKERHUB_TOKEN` | Docker Hub Personal Access Token | [Create PAT](https://hub.docker.com/settings/security) with `repo:delete` scope |
...
```bash
podman manifest inspect ghcr.io/<username>/mcp-taipei-metro-month-price:latest
```
...
- If credentials invalid: Regenerate PAT with `repo:delete` scope
