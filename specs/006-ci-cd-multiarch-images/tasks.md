...
- [X] T008 Define job structure: `jobs: build-and-push: runs-on: ubuntu-latest` in `.github/workflows/build-images.yml`
- [X] T009 Add checkout step: `uses: actions/checkout@v4` in `.github/workflows/build-images.yml`
- [X] T010 [P] Add QEMU setup step: `uses: docker/setup-qemu-action@v3` for multi-arch emulation in `.github/workflows/build-images.yml`
...
**Phase 2 (Foundation)**:
- T006 (concurrency), T007 (env vars), T010 (QEMU) can run in parallel after T005
